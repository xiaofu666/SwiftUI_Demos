...
float2 snow_hash( float2 p ) { p=float2(dot(p,float2(127.1, 311.7)),dot(p,float2(269.5,183.3))); return fract(sin(p)*18.5453); }
...
float simplegridnoise(float2 v, float T)
{
    ...
    for(int y = -1; y <= 1; y++)
        for(int x = -1; x <= 1;x++)
        {
            ...
            float2 pos = .5 + .5 * cos(2.* pi * (T*.1 + snow_hash(fl+offset)) + float2(0,1.6));
