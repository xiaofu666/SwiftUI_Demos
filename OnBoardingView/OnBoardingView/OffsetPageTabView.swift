...
struct OffsetPageTabView<Content: View>: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UIScrollView{
        ...
        scrollView.setContentOffset(CGPoint(x: CGFloat(selection) * getScreenRect().width, y: 0), animated: true)
        return scrollView
    }
    ...
    func updateUIView(_ uiView: UIScrollView, context: Context) {
        ...
        if uiView.bounds.size.width > 0.0 && Int((currentOffset / uiView.bounds.size.width).rounded()) != selection {
            
            print("updating selection")
            uiView.setContentOffset(CGPoint(x: CGFloat(selection) * uiView.bounds.size.width, y: 0), animated: true)
        }
    }
    ...
    class Coordinator: NSObject, UIScrollViewDelegate {
        ...
        func scrollViewDidScroll(_ scrollView: UIScrollView) {
            ...
            let maxSize = scrollView.bounds.width
